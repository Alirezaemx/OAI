      ...
      for (int qos_flow_index = 0; qos_flow_index < ue->pduSession[i].param.nb_qos; qos_flow_index++) {
        switch (ue->pduSession[i].param.qos[qos_flow_index].fiveQI) {
          case 1 ... 4: /* GBR */
            ...
            break;
          case 5 ... 9: /* Non-GBR */
            if (rrc->configuration.drbs
                > 1) /* Force the creation from gNB Conf file - Should be used only in noS1 mode and rfsim for testing purposes. */
              drb_id = next_available_drb(ue, &ue->pduSession[i], GBR_FLOW);
            else
...
static void apply_macrlc_config(gNB_RRC_INST *rrc, rrc_gNB_ue_context_t *const ue_context_pP, const protocol_ctxt_t *const ctxt_pP)
{
  ...
  NR_CellGroupConfig_t *cgc = get_softmodem_params()->sa ? ue->masterCellGroup : NULL;
  nr_rrc_mac_update_cellgroup(ue->rnti, cgc);

  nr_rrc_addmod_srbs(ctxt_pP->rntiMaybeUEid, ue->SRB_configList, cgc->rlc_BearerToAddModList);
  NR_DRB_ToAddModList_t *DRBs = fill_DRB_configList(ue);
...
//-----------------------------------------------------------------------------
{
  ...
  gNB_RRC_UE_t *ue_p = &ue_context_pP->ue_context;
  NR_DRB_ToAddModList_t *DRB_configList = fill_DRB_configList(ue_p);
  int nb_drb_to_setup = 0;
  if (DRB_configList)
    nb_drb_to_setup = DRB_configList->list.count;
