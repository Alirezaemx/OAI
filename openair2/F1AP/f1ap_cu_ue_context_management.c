  ...
  if(f1ap_ue_context_setup_req->drbs_to_be_setup_length){
    ...
    for (int i = 0; i < f1ap_ue_context_setup_req->drbs_to_be_setup_length; i++) {
      ...
      for (int j = 0; j < f1ap_ue_context_setup_req->drbs_to_be_setup[i].up_ul_tnl_length; j++) {
        ...
        f1_inst->rnti_update.prior_rnti = f1ap_ue_context_setup_req->rnti;
        f1_inst->rnti_update.new_rnti = 0;
        f1ap_ue_context_setup_req->drbs_to_be_setup[i].up_ul_tnl[j].teid = newGtpuCreateTunnel(f1_inst->gtpInst,
...
int CU_handle_UE_CONTEXT_SETUP_RESPONSE(instance_t       instance,
  ...
  f1ap_cudu_inst_t *f1_inst = getCxt(CUtype, instance);
  if (f1ap_ue_context_setup_resp->rnti == (uint16_t)(-1)) {
    ...
    if (ie == NULL) {
      ...
      f1ap_ue_context_setup_resp->rnti = ie->value.choice.C_RNTI;
      int f1ap_uid = f1ap_add_ue(CUtype, instance, ie->value.choice.C_RNTI);
      if (f1ap_uid  < 0 ) {
        LOG_E(F1AP, "Failed to add UE \n");
        return -1;
      }
      f1_inst->rnti_update.new_rnti = f1ap_ue_context_setup_resp->rnti;
      f1ap_ue_context_setup_resp->crnti = calloc(1, sizeof(uint16_t));
      *f1ap_ue_context_setup_resp->crnti = f1ap_ue_context_setup_resp->rnti;
      f1ap_ue_context_setup_resp->gNB_CU_ue_id = f1ap_get_cu_ue_f1ap_id(CUtype, instance, ie->value.choice.C_RNTI);
      f1ap_ue_context_setup_resp->gNB_DU_ue_id = f1ap_get_du_ue_f1ap_id(CUtype, instance, ie->value.choice.C_RNTI);
    }
  }
  ...
  if(ie!=NULL) {
    ...
    for (i = 0; i < f1ap_ue_context_setup_resp->drbs_to_be_setup_length; ++i) {
      ...
      AssertFatal(drbs_setup_item_p->dLUPTNLInformation_ToBeSetup_List.list.count > 0, "no DL UP TNL Information in DRBs to be Setup list\n");
      ...
      if (f1_inst->rnti_update.new_rnti != 0) {
        GtpuUpdateTunnelRNTI(f1_inst->gtpInst, f1_inst->rnti_update.prior_rnti, f1_inst->rnti_update.new_rnti);
      }
      GtpuUpdateTunnelOutgoingAddressAndTeid(f1_inst->gtpInst, f1ap_ue_context_setup_resp->rnti, (ebi_t)drbs_setup_item_p->dRBID, drb_p->up_dl_tnl[0].tl_address, drb_p->up_dl_tnl[0].teid);
    }
  }
      ...
      if(f1ap_ue_context_modification_req->QoS_information_type == EUTRAN_QoS){
        ...
        for (int j=0;  j<maxnoofULTunnels;  j++) {
          ...
          asn1cSequenceAdd( drbs_toBeSetupMod_item->uLUPTNLInformation_ToBeSetup_List.list,
                        F1AP_ULUPTNLInformation_ToBeSetup_Item_t, uLUPTNLInformation_ToBeSetup_Item);
          uLUPTNLInformation_ToBeSetup_Item->uLUPTNLInformation.present = F1AP_UPTransportLayerInformation_PR_gTPTunnel;
          asn1cCalloc(uLUPTNLInformation_ToBeSetup_Item->uLUPTNLInformation.choice.gTPTunnel, gTPTunnel);
