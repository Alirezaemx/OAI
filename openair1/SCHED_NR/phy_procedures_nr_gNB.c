...
void phy_procedures_gNB_TX(processingData_L1tx_t *msgTx,
  ...
  for (int i = 0; i < NR_SYMBOLS_PER_SLOT; i++){
    ...
    if (csirs->active == 1) {
      ...
      nr_generate_csi_rs(&gNB->frame_parms, (int32_t **)gNB->common_vars.txdataF, gNB->TX_AMP, gNB->nr_csi_info, csi_params, slot, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
      csirs->active = 0;
    }
  }
//  if ((frame&127) == 0) dump_pdsch_stats(gNB);
  ...
  if (gNB->num_RU==0 || ((gNB->num_RU >= 1) && (gNB->RU_list[0]->if_south != REMOTE_IF4p5))) {
    for (aa = 0; aa < cfg->carrier_config.num_tx_ant.value; aa++) {
