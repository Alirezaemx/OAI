...
//  nr_phy_config_request_sim(gNB,N_RB_DL,N_RB_DL,mu,0,0x01);
  ...
  for (SNR = snr0; SNR <= snr1; SNR += snr_step) {
    ...
    for (trial = 0; trial < n_trials; trial++) {
      ...
      min_pusch_delay = ulsch_gNB->delay.est_delay < min_pusch_delay ? ulsch_gNB->delay.est_delay : min_pusch_delay;
      max_pusch_delay = ulsch_gNB->delay.est_delay > max_pusch_delay ? ulsch_gNB->delay.est_delay : max_pusch_delay;
