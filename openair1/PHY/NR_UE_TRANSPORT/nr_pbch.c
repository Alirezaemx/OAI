...
static uint16_t nr_pbch_extract(const NR_DL_FRAME_PARMS *frame_parms,
                                ...
                                const struct complex16 dl_ch_estimates[][frame_parms->ofdm_symbol_size],
...
static void nr_pbch_channel_compensation(const struct complex16 rxdataF_ext[][PBCH_MAX_RE_PER_SYMBOL],
                         ...
                         int16_t pbch_e_rx[NR_POLAR_PBCH_E])
{
  ...
  __attribute__ ((aligned(32))) struct complex16 rxdataF_ext[ue->frame_parms.nb_antennas_rx][PBCH_MAX_RE_PER_SYMBOL];
  __attribute__ ((aligned(32))) struct complex16 dl_ch_estimates_ext[ue->frame_parms.nb_antennas_rx][PBCH_MAX_RE_PER_SYMBOL];
  memset(dl_ch_estimates_ext,0, sizeof  dl_ch_estimates_ext);

  nr_pbch_extract(&ue->frame_parms,
                  rxdataF,
                  symbolPbch,
                  symbol_offset,
                  dl_ch_estimates,
  ...
  double log2_maxh = 0;
  if (symbolPbch == 1) {
    const int max_h = nr_pbch_channel_level(dl_ch_estimates_ext,
                                            &ue->frame_parms,
                                            nb_re);
...
#endif
  __attribute__ ((aligned(32))) struct complex16 rxdataF_comp[ue->frame_parms.nb_antennas_rx][PBCH_MAX_RE_PER_SYMBOL];
  ...
  */
  ...
  if (symbolPbch == 1) {
    pbch_e_rx_idx = 0;
  } else if (symbolPbch == 2) {
    pbch_e_rx_idx = 360;
  } else if (symbolPbch == 3) {
    pbch_e_rx_idx = 360 + 144;
  }

  const int nb = (symbolPbch == 2) ? 144 : 360;
                   ...
                   nr_phy_data_t *phy_data)
{
  ...
  uint64_t tmp = 0;
  const int decoderState = polar_decoder_int16(pbch_e_rx,(uint64_t *)&tmp,0,
  ...
  const int M = (ue->frame_parms.Lmax == 64)? (NR_POLAR_PBCH_PAYLOAD_BITS - 6) : (NR_POLAR_PBCH_PAYLOAD_BITS - 3);
  nushift = ((pbch_a_prime>>24)&1) ^ (((pbch_a_prime>>6)&1)<<1);
  pbch_a_interleaved=0;
  nr_pbch_unscrambling(pbch_e_rx, ue->frame_parms.Nid_cell, nushift, M, NR_POLAR_PBCH_PAYLOAD_BITS,
               ...
               c16_t **rxdataF[NR_SYMBOLS_PER_SLOT])
{
  ...
  write_output("rxdataF0_pbch.m","rxF0pbch",
               rxdataF[symbol_offset+1][0],frame_parms->ofdm_symbol_size,1,1);
#endif
