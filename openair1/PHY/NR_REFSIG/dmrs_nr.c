...
int get_last_dmrs_symbol_in_slot(const int dmrs_symbol_pos)
{
  for (int i = NR_SYMBOLS_PER_SLOT-1; i > 0; i--) {
    if ((dmrs_symbol_pos >> i) & 0x1) return i;
  }
  return -1;
}
...
void nr_chest_time_domain_avg(const NR_DL_FRAME_PARMS *frame_parms,
                              ...
                              const int layerIdx,
                              const int Nl,
                              c16_t ch_estimates[NR_SYMBOLS_PER_SLOT]
                                                [Nl]
                                                ...
                                                [frame_parms->ofdm_symbol_size])
{
  ...
  for (int symb = first_dmrs_symb+1; symb < total_symbols; symb++) {
    ul_ch128_0 = (__m128i *)ch_estimates[first_dmrs_symb][layerIdx][antIdx];
    if ((dmrs_bitmap >> symb) & 0x01) {
      ul_ch128_1 = (__m128i *)ch_estimates[symb][layerIdx][antIdx];
  ...
  if (num_dmrs_symb == 2) {
    ...
    ul_ch16_0 = (int16_t *)ch_estimates[first_dmrs_symb][layerIdx][antIdx];
    for (int rbIdx = 0; rbIdx < num_rbs; rbIdx++) {
      ul_ch16_0[0] /= 3;
      ul_ch16_0[1] /= 3;
      ul_ch16_0[2] /= 3;
