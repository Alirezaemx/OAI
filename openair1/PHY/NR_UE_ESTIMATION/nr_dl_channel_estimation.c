...
void nr_est_delay_pdsch(const NR_DL_FRAME_PARMS *frame_parms, const c16_t *dl_ls_est, NR_delay_t *delay)
{
  c16_t dl_ch_estimates_time[frame_parms->ofdm_symbol_size];
  ...
  for (int i = 0; i < frame_parms->ofdm_symbol_size; i++) {
    int temp = (dl_ch_estimates_time[i].r * dl_ch_estimates_time[i].r / 2) + (dl_ch_estimates_time[i].i * dl_ch_estimates_time[i].i / 2);
    if (temp > max_val) {
      max_pos = i;
      max_val = temp;
    }
                                       ...
                                       NR_delay_t *delay)
{
  ...
  memset(dl_ls_est, 0, sizeof(c16_t) * frame_parms->ofdm_symbol_size);
                                ...
                                c16_t rxdataF[][rxdataFsize])
{
  ...
  NR_delay_t delay;
  memset(&delay, 0, sizeof(NR_delay_t));
